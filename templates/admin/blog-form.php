...
<div class="flex min-h-screen">
    ...
    <aside id="admin-sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-slate-900 dark:bg-slate-950 flex flex-col transition-transform duration-300 -translate-x-full lg:translate-x-0">
        <div class="flex items-center gap-2.5 px-5 py-4 border-b border-slate-800">
            <span class="flex items-center justify-center w-8 h-8 rounded-lg bg-brand-600 text-white shrink-0"><i data-lucide="shield-alert" class="w-4 h-4"></i></span>
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
            ...
            <a href="<?= url('/admin/blog') ?>" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium bg-slate-800 text-white"><i data-lucide="file-pen-line" class="w-4 h-4 shrink-0"></i> Artículos</a>
            <?php endif; ?>
            <?php if ($user->esAdmin()): ?>
            ...
            <a href="<?= url('/admin/comentarios') ?>" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800/50 transition-colors"><i data-lucide="message-circle" class="w-4 h-4 shrink-0"></i> Comentarios</a>
            <?php endif; ?>
            <?php if ($user->esAdmin() || $user->role === 'revisor'): ?>
